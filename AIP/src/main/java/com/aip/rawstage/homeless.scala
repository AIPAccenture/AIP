...
object homeless {
  def main(args: Array[String]): Unit = {
    ...
    val src_file = Property.getProperty("src_homeless")
    val des_file = Property.getProperty("des_homeless")
    ...
    df1.repartition(1).write.format("com.databricks.spark.csv").option("delimiter", "\t").option("header", "false").save(des_file)
